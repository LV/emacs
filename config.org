...
#+begin_src emacs-lisp
  ...
  If at a bullet point:
    - If empty, delete the bullet and insert a newline.
    - If non-empty, insert a new bullet.
  Else, insert a newline."
      (interactive)
      (cond
       ;; If at an empty bullet point, remove it and insert a newline
       ((and (org-at-item-p)
             (save-excursion
               (beginning-of-line)
               (looking-at-p "^[ \t]*[-+*] \\(?:[ \t]*$\\)")))
...
#+begin_src emacs-lisp
  (lv/leader-keys
    ...
    "bb" '(evil-switch-to-windows-last-buffer  :which-key "back to last buffer")
