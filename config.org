...
#+begin_src emacs-lisp
  ...
  (defun lv/setup-git-commit-mode-evil-commands ()
    ...
    (evil-ex-define-cmd "w[rite]" #'lv/magit-commit-save-and-exit)
...
#+begin_src emacs-lisp
  (defun my-org-new-file-template ()
  ...
  (add-hook 'find-file-hook 'my-org-new-file-template)
#+end_src

I want the ~#+last_modified~ keyword to automatically update upon saving
...
#+begin_src emacs-lisp
  (lv/leader-keys
    ...
    "ode" '(org-roam-dailies-capture-today :which-key "entry")
