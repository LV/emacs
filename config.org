...
As per this [[https://zzamboni.org/post/beautifying-org-mode-in-emacs/][blog post]]

Hide ~/~ symbol when writing /emphasis/
#+BEGIN_SRC emacs-lisp
...
Use the [[https://edwardtufte.github.io/et-book/][ET Book]] font
#+BEGIN_SRC emacs-lisp
  (let* ((variable-tuple
    ...
    (custom-theme-set-faces
     ...
     `(org-code             ((t (,@code-font))))    ; ~code~
     `(org-verbatim         ((t (,@code-font))))))  ; =verbatim=
#+END_SRC

Set up ~variable-pitch~ and ~fixed-pitch~ faces
#+BEGIN_SRC emacs-lisp
  (custom-theme-set-faces
   'user
   '(variable-pitch ((t (:family "ETBembo" :height 180 :weight thin))))
   '(fixed-pitch ((t ( :family "PragmataPro Mono Liga" :height 150)))))
#+END_SRC
...
Configure faces for specific Org element
#+BEGIN_SRC emacs-lisp
  (custom-theme-set-faces
   'user
